...
class PagureNotifications(Plugin):    
    ...
    async def handle_request(self, request):
        ...
        self.log.error(jsonlib.dumps(json, indent=2))
        ...
        if project_name not in self.config["projects"]:
            self.log.error(f'project {project_name} is sending me a webhook, but it is not defined in my config!')
            return Response()
        ...
        if not hmac.compare_digest(hashhex, request.headers.get('X-Pagure-Signature')):
            self.log.error(f'message from project {project_name} did not validate correctly. ignoring')
            return Response()
